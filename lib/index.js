...
export default function slackin({
  ...
  if(!sitekey || !captcha) throw new Error('Must provide recaptcha secret')
  ...
  app.get('/', (req, res) => {
    ...
    let page = dom('html',
      ...
      splash({ coc, path, css, name, org, logo, channels, active, total, sitekey })
    )
    res.type('html')
  ...
  app.post('/invite', json(), (req, res, next) => {
    verifyRecaptcha(captcha, req.body.recaptcha, err => {
      if (err) {
        return res
          .status(400)
